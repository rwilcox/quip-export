...
class QuipService {
    ...
    async _apiCall(url, method = 'GET', blob) {
        this.logger.info(`APICALL: URL=${url}, flag=${this.timelimit}`);
        if(this.timelimit) {
            return this._repeatCall(url, method, blob);
        }
        ...
        try {
            ...
            if(+res.headers.get('x-ratelimit-remaining') === 0) {
                ...
                setTimeout(() => {
                    this.timelimit = false
                }, res.headers.get('x-ratelimit-reset')*1000 - new Date().getTime());
                return this._repeatCall(url, method, blob);
            }
