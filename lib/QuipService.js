...
class QuipService {
    ...
    _repeatCallWithDelay(ms, url, method, blob) {
        this.logger.debug(`URL: ${url} waiting in ms: ${ms}`);
    ...
    async _apiCall(url, method = 'GET', blob) {
        if(this.delayMs > 0) {
            return this._repeatCallWithDelay(this.delayMs, url, method, blob);
        }

        try {
            this.stats.query_count++;
            const res = await fetch(`${this.apiURL}${url}`, this._getOptions(method));
            this.logger.info(`X-Ratelimit-Remaining: ${res.headers.get('x-ratelimit-remaining')}`);
            if(+res.headers.get('x-ratelimit-remaining') === 0) {
                this.logger.debug(`X-Ratelimit-Remaining = 0`);
                this.delayMs = res.headers.get('x-ratelimit-reset')*1000 - new Date().getTime();
                this.logger.info(`**********${this.delayMs}`);
                return this._repeatCallWithDelay(this.delayMs, url, method, blob);
            }
            //this.logger.info(`X-Ratelimit-Limit: ${res.headers.get('x-ratelimit-limit')},
            //                    X-Ratelimit-Remaining: ${res.headers.get('x-ratelimit-remaining')},
            //                    X-Ratelimit-Reset: ${res.headers.get('x-ratelimit-reset')*1000 - new Date().getTime()}`);
            if(!res.ok) {
                if(res.status === 503) {
                    this.logger.debug(`HTTP 503: for ${url}, waiting in ms: ${this.delayMs}`);
                    if(this._check503Query(url)) {
                        return this._repeatCallWithDelay(url, method, blob);
                    } else {
