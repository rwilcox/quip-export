...
class QuipService {
    ...
    async _apiCall(url, method = 'GET', blob) {
        if(this.delayUntil >= currentTime) {
            ...
            this.logger.debug(`APICALL: URL=${url}, waiting ${this.delayUntil - currentTime} ms...`);
        ...
        try {
            ...
            if(+res.headers.get('x-ratelimit-remaining') === 0) {
                if(rateLimitReset - currentTime > 0) {
                    this.delayUntil = rateLimitReset;
                } else {
                    this.delayUntil = currentTime + 1000; //1 second
                }
                this.logger.debug(`x-ratelimit-remaining=0: for ${url}, set up delay ms: ${this.delayUntil - new Date().getTime()}`);
            ...
            if(!res.ok) {
                if(res.status === 503) {
                    if(this._check503Query(url)) {
                        ...
                        this.logger.debug(`HTTP 503: for ${url}, set up delay ms: ${this.delayUntil - new Date().getTime()}`);
                        return this._apiCall(url, method, blob);
                    } else {
