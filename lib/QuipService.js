...
class QuipService {
    ...
    async _apiCall(url, method = 'GET', blob) {
        ...
        if(this.delayUntil >= currentTime) {
            this.logger.info(`APICALL: URL=${url}, waiting...`);
            return new Promise(resolve => setTimeout(() => {
                resolve(this._apiCall(url, method, blob));
            }, this.delayUntil - currentTime));
        }

        try {
            this.stats.query_count++;
            const res = await fetch(`${this.apiURL}${url}`, this._getOptions(method));
            this.logger.info(`X-Ratelimit-Remaining: ${res.headers.get('x-ratelimit-remaining')}, URL=${url}, flag=${this.timelimit}`);
            if(+res.headers.get('x-ratelimit-remaining') === 0) {
                this.logger.debug(`Waiting for ms=${res.headers.get('x-ratelimit-reset')*1000 - new Date().getTime()}`);
                this.delayUntil = res.headers.get('x-ratelimit-reset')*1000;
                return this._apiCall(url, method, blob);
            }
